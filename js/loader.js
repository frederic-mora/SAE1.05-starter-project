/*
    ...
    Ce fichier contient deux fonctions "outils" qui vous permettent de charger
    des fichiers HTML contenant des templates de composants, et des fichiers JSON
    qui contiennent les données à utiliser pour formater les templates.
...
/*  loadJSON
    @param filename: string, le chemin vers le fichier JSON à charger
    @return object, le contenu du fichier JSON (sous forme d'objet JS)
*/
let loadJSON = async function(filename) {
    ...
    let response = await fetch(filename);
    // response est un objet JS qui contient le résultat de la lecture du fichier
    // response.json() est une fonction qui permet de récupérer le contenu du fichier JSON et de le convertir en objet JS
    // c'est aussi une opération qui n'est pas instantanée, il faut donc attendre (await) que la conversion soit terminée
    let obj = await response.json();
    // on retourne le contenu du fichier JSON sous forme d'objet JS
    return obj;
}

// on exporte les deux fonctions pour pouvoir les utiliser dans d'autres modules JS
